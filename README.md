...
│   ├── shixiseng.py          # 实习僧爬虫（列表页 + 详情页 JD，含相关性过滤）
│   └── zhilian.py            # 智联招聘爬虫（列表页 + 详情页 JD）
...
### 实习僧岗位过滤
实习僧搜索结果经过严格的**相关性过滤**，仅保留岗位名称中包含"数据分析、数据运营、数据挖掘、BI、商业分析"等数据相关关键词的岗位，排除不相关职位。
...
2. **详情页访问耗时**: 三个平台均需逐个访问详情页获取完整 JD，10 页数据量可能需要较长时间（~30 分钟）。

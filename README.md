...
> 自动爬取"牛客网、智联招聘"两大平台的**数据分析**岗位，按**杭州、宁波、绍兴、湖州、嘉兴、南京、苏州、扬州、上海**共 9 个城市逐城市 × 逐职位类型抓取，工作地点使用**具体城市名称**，清洗后**清空飞书旧数据并全量写入**新数据到**飞书多维表格（Bitable）**。杭州和上海在智联招聘上最多抓取 5 页。支持 GitHub Actions 每日定时运行。
...
| 省份/直辖市 | 城市 |
|-----------|------|
| 浙江省（5市）| 杭州、宁波、绍兴、湖州、嘉兴 |
| 江苏省（3市）| 南京、苏州、扬州 |
| 直辖市（1市）| 上海 |

**总计 9 个城市**，工作地点使用具体城市名称。
...
| 智联招聘 | 全职、兼职/临时、实习、校园 | 杭州/上海最多 5 页，其余城市第 1 页 |

- 牛客网: 9 城市 × 3 类型 × 1 页 = **27 次搜索**
- 智联招聘: 7 城市 × 4 类型 × 1 页 + 2 城市 × 4 类型 × 最多 5 页 = **最多 68 次搜索**
- 总计最多约 **95 次搜索**（加上详情页访问）
...
2. **9 个城市**: 浙江省 5 市（杭州、宁波、绍兴、湖州、嘉兴）+ 江苏省 3 市（南京、苏州、扬州）+ 上海。
3. **智联多页抓取**: 杭州和上海在智联招聘上最多抓取 5 页（无结果时自动停止），其余城市仅第 1 页。
4. **飞书先清空再写入**: 每次运行时先删除飞书表格中所有旧记录，再从第 1 行开始全量写入新数据。
5. **牛客登录弹窗**: 牛客网未登录状态下会弹出登录对话框阻止 UI 交互，爬虫通过 JS 隐藏弹窗解决。未登录时翻页内容重复，因此每城市仅抓取第 1 页。

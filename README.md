...
> 自动爬取"牛客网、智联招聘"两大平台的**数据分析**岗位，按**浙江省、江苏省、广东省各地级市及北京、上海**逐城市 × 逐职位类型抓取，清洗后同步至**飞书多维表格（Bitable）**。支持 GitHub Actions 每日定时运行。
...
| 省份/直辖市 | 城市 |
|-----------|------|
| 浙江省（11市）| 杭州、宁波、温州、嘉兴、湖州、绍兴、金华、衢州、舟山、台州、丽水 |
| 江苏省（13市）| 南京、无锡、徐州、常州、苏州、南通、连云港、淮安、盐城、扬州、镇江、泰州、宿迁 |
| 广东省（21市）| 广州、深圳、珠海、汕头、佛山、韶关、湛江、肇庆、江门、茂名、惠州、梅州、汕尾、河源、阳江、清远、东莞、中山、潮州、揭阳、云浮 |
| 直辖市（2市）| 北京、上海 |
...
每个城市 × 每个职位类型 = 1 页结果。牛客网 47 × 3 = 141 次搜索，智联招聘 47 × 3 = 141 次搜索，总计最多 **282 次搜索**（加上详情页访问）。
...
1. **按城市 × 职位类型抓取**: 牛客网通过页面城市级联筛选器逐城市勾选，同时分别选择"实习"/"校招"/"社招"标签页；智联招聘通过 URL `jl` 参数指定城市，再通过"职位类型"筛选下拉框分别选择"全职"/"兼职"/"实习"。
2. **47 个城市 × 3 种类型 × 2 个平台**: 总计最多 282 次搜索加上详情页访问，完整运行需要较长时间。
3. **牛客登录弹窗**: 牛客网未登录状态下会弹出登录对话框阻止 UI 交互，爬虫通过 JS 隐藏弹窗解决。未登录时翻页内容重复，因此每城市仅抓取第 1 页。
4. **选择器可能更新**: 招聘网站前端会不定期改版，如果爬虫失效，需要检查并更新 CSS 选择器。
5. **飞书 API 限流**: 飞书开放平台有频率限制，批量写入时已做分批处理（每批 500 条）。
6. **数据合规**: 请确保爬取行为符合各平台的使用条款和相关法律法规，仅供个人学习使用。
7. **CSV 输出**: 每次运行产生的 CSV 文件保存在 `output/` 目录下，文件名包含时间戳。

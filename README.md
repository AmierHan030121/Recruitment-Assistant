...
> 自动爬取"牛客网、智联招聘"两大平台的**数据分析**岗位，按**浙江省、江苏省、广东省各地级市及北京、上海**逐城市抓取，清洗后同步至**飞书多维表格（Bitable）**。支持 GitHub Actions 每日定时运行。
...
│   ├── nowcoder.py           # 牛客网爬虫（按城市搜索 + 详情页 JD）
│   └── zhilian.py            # 智联招聘爬虫（按城市搜索 + 详情页 JD）
├── cleaner.py                # 数据清洗模块（城市标准化、关键词提取）
├── config.py                 # 配置模块（API 密钥、关键词、目标城市列表）
...
1. **按城市抓取**: 每个城市每个平台仅抓取第 1 页结果。牛客网未登录时翻页内容重复，因此改为按城市搜索。
2. **47 个城市 × 2 个平台**: 总计最多 94 次搜索，加上详情页访问，完整运行可能需要较长时间。
3. **选择器可能更新**: 招聘网站前端会不定期改版，如果爬虫失效，需要检查并更新 CSS 选择器。
4. **飞书 API 限流**: 飞书开放平台有频率限制，批量写入时已做分批处理（每批 500 条）。
5. **数据合规**: 请确保爬取行为符合各平台的使用条款和相关法律法规，仅供个人学习使用。
6. **CSV 输出**: 每次运行产生的 CSV 文件保存在 `output/` 目录下，文件名包含时间戳。
